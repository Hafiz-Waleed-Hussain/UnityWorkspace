...
public class BWDiagonalBackgroundLayer : MonoBehaviour {
	...
	private void checkPositionForObject(Transform obj, Transform next, Transform prev) {
		...
		if (obj.position.x <= -1.5f*obj.localScale.x) {
			Vector3 pos = obj.position;
			pos.x = prev.position.x + prev.localScale.x - 2;
			pos.y = prev.position.y + offset;
			obj.position = pos;
    	} else if (obj.position.x >= 1.5f*obj.localScale.x) {
			Vector3 pos = obj.position;
			pos.x = next.position.x + 2 - obj.localScale.x;
	...
	private void checkPositionForTrees(Transform obj, Transform next, Transform prev) {
		float offset = (float)(Math.Tan(BWConstants.diagonalAngle)*1200.0f);
		if (obj.position.x <= -1.5f*1200.0f) {
			Vector3 pos = obj.position;
			pos.x = prev.position.x + 1200 - 2;
			pos.y = prev.position.y + offset;
			obj.position = pos;
    	} else if (obj.position.x >= 1.5f*1200.0f) {
			Vector3 pos = obj.position;
			pos.x = next.position.x + 2 - 1200;
			pos.y = next.position.y - offset;
			obj.position = pos;
    	}
	}
