...
public class CircusWorldMenu : MonoBehaviour {
	...
	void FingerGestures_OnFingerTap( int fingerIndex, Vector2 fingerPos, int tapCount )
    {
		...
		if(selection.name.StartsWith("Icon1")) {
			showAlertWithImage("unlockpopup_tracinglines");
		}
		
		if(selection.name.StartsWith("Icon2")) {
			showAlertWithImage("unlockpopup_tracingletters");
		}
		
		if(selection.name.StartsWith("Icon3")) {
			showAlertWithImage("unlockpopup_alphabetgame");
		}
		
		if(selection.name.StartsWith("Icon4")) {
			showAlertWithImage("unlockpopup_stacksorting");
		}
		
		if(selection.name.StartsWith("Icon5")) {
			showAlertWithImage("unlockpopup_abchidenseek");
		}
		
		if(selection.name.StartsWith("Icon6")) {
			showAlertWithImage("unlockpopup_soundgallery");
		}
    }
