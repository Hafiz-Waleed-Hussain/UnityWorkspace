...
private var line : NF_TubeRenderer;
		...
		if(rope) {
		 ...
		 line.enabled = true;
			...
			// segmentPos[s] = joints[s].transform.position;
			 segmentPos[s].x = joints[s].transform.position.x;
			 segmentPos[s].y = joints[s].transform.position.y;
...
function BuildRope()
{
	...
	line = tubeRenderer.AddComponent(NF_TubeRenderer);
